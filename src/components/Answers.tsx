...
const Answers = ({
	...
	const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
		...
		try {
			...
			window.alert(error?.message || "Error in creating answers");
		}
	};
	...
	const deleteAnswer = async (answerId: string) => {
		try {
			...
			setAnswers((prev) => ({
				total: prev.total - 1,
				documents: prev.documents.filter((doc) => doc.$id !== answerId),
			}));
		} catch (error: any) {
			window.alert(error?.message || "Error in deleting answers");
		}
	};
